...
void print_line_npu(int line_num, int op_type, int size_type, uint64_t addr, uint64_t data_low, uint64_t data_high) {
    printf("Line %d: %s %s 0x%016lx",
                   line_num,
                   op_type == LOAD ? "read" : "store",
                   size_type == BYTE ? "byte" : 
                   size_type == HALF ? "halfword" :
                   size_type == WORD ? "word" : 
                   size_type == DOUBLE ? "doubleword" : "quadword",
                   addr);
    
    // 对于quadword，输出128位数据
    if (size_type == QUAD) {
        printf(" 0x%016lx%016lx", data_high, data_low);
    } else {
        printf(" 0x%016lx", data_low);
    }
...
void cache_line_npu(FILE* ch0, FILE* ch1, FILE* ch2, const char* line, int op_type, int size_type, uint64_t addr) {
    ...
    else if (ch2 && line && (size_type == QUAD) && (op_type==LOAD)) {
        fputs(line, ch2);
        fflush(ch2);  // ensure writing immediately

    }
    else if (ch1 && line && (op_type==LOAD)) {
        fputs(line, ch1);
        fflush(ch1);  // ensure writing immediately
    }
}
...
int handle_multiport_rsp_data_npu (FILE* ch0, FILE* ch1, FILE* ch2, int* ref_line_num, int* rsp_retry_num, bool req_end) {
    // save current file pos
    long ch0_current_pos = ftell(ch0);
    long ch1_current_pos = ftell(ch1);
    long ch2_current_pos = ftell(ch2);
    ...
    if (top->u_channel_0_rsp_bus_valid && top->u_channel_0_rsp_bus_ready) {
        ...
        int req_type, op_type, size_type;
        uint64_t ref_addr, ref_data_low, ref_data_high;
    
        if(fgets(line, sizeof(line), ch0)){
            *ref_line_num++;      
            parse_trace_line_npu(line, &req_type, &op_type, &size_type, &ref_addr, &ref_data_low, &ref_data_high);
        }
        // compare dut_data with ref
        int data_match = 0;
            
        if (size_type == BYTE) {
            data_match = ((dut_data_low & 0xFF) == (ref_data_low & 0xFF));
        } else if (size_type == HALF) {
            data_match = ((dut_data_low & 0xFFFF) == (ref_data_low & 0xFFFF));
        } else if (size_type == WORD) {
            data_match = ((dut_data_low & 0xFFFFFFFF) == (ref_data_low & 0xFFFFFFFF));
        } else if (size_type = DOUBLE) {
            data_match = (dut_data_high == ref_data_high && dut_data_low == ref_data_low);
        }

        if (data_match) {
            printf("DIFFTEST: CH0 PASS - Addr=0x%016lx, Data=0x%016lx%016lx\n", ref_addr, ref_data_high, ref_data_low);
            ch0_state = PASS;
        } else {
            printf("DIFFTEST: CH0 FAIL - Addr=0x%016lx\n", ref_addr);
            printf("  Expected: 0x%016lx%016lx\n", ref_data_high, ref_data_low);
            printf("  Got:      0x%016lx%016lx\n", dut_data_high, dut_data_low);
            printf("  Trace line: %d\n", *ref_line_num);
            ch0_state = FAIL;
        }
    ...
    if (top->u_channel_1_rsp_bus_valid && top->u_channel_1_rsp_bus_ready) {
        ...
        if (data_match) {
            printf("DIFFTEST: CH1 PASS - Addr=0x%016lx, Data=0x%016lx%016lx\n", ref_addr, ref_data_high, ref_data_low);
            ch1_state = PASS;
        } else {
            printf("DIFFTEST: CH1 FAIL - Addr=0x%016lx\n", ref_addr);
            printf("  Expected: 0x%016lx%016lx\n", ref_data_high, ref_data_low);
    ...
    if (top->u_channel_2_rsp_bus_valid && top->u_channel_2_rsp_bus_ready) {
        ...
        int req_type, op_type, size_type;
        uint64_t ref_addr, ref_data_high, ref_data_low;
        ...
        if (data_match) {
            printf("DIFFTEST: CH2 PASS - Addr=0x%016lx, Data=0x%016lx%016lx\n", ref_addr, dut_data_high, dut_data_low);
            ch2_state = PASS;
        } else {
            printf("DIFFTEST: CH2 FAIL - Addr=0x%016lx\n", ref_addr);
            printf("  Expected: 0x%016lx%016lx\n", ref_data_high, ref_data_low);

...
void update_mshr_stat(uint64_t slice0, uint64_t slice1, uint64_t slice2, uint64_t slice3) {
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (parse_trace_line(line, &req_type, &op_type, &size_type, &addr, &data)) {
            ...
            while (retry_count < MAX_RETRY && !(top->u_channel_0_req_bus_valid && top->u_channel_0_req_bus_ready ||
                ...
                event->cycles++;
                // update pmu
                update_mshr_stat(top->slice_0_pmu_lsq_busy, top->slice_1_pmu_lsq_busy, top->slice_2_pmu_lsq_busy, top->slice_3_pmu_lsq_busy);
...
// difftest
    ...
    if (rsp == DONE) {
        ...
        print_perf_stats(event, line_num);
        printf("\n");
    }
    if (rsp == DEADLOCK) {
        printf("\n%s%s%s\n", COLOR_RED, SEPARATOR, COLOR_RESET);
